...
public class AndroidDriverBase extends AndroidDriver<AndroidElement> {
	...
	public static Map<String, String> inputs = new HashMap<String, String>();
	...
	public AndroidDriverBase(String server,String port,String capsPath,String udid,String input) throws Exception{
		...
		AndroidDriverBase.inputs.put(udid, input);
	}
	...
	public AndroidElement findElement(By by){
		try {
			//			new AndroidDriverWait(this,5).until(ExpectedCondition.presenceOfElementLocated(by));//这是应为隐式等待不起效果才加的代码
	 ...
	 * 与服务端断开连接,断开前重置输入法
	 */
	public void quit(String udid){
		this.resetInput(udid);
		super.quit();
	}
	...
	public void implicitlyWait(int timeout){
		...
		System.out.println(super.manage().timeouts().implicitlyWait(timeout, TimeUnit.SECONDS));
	}
	...
	public void pageLoadTimeout(int timeout){
		super.manage().timeouts().pageLoadTimeout(timeout, TimeUnit.SECONDS);
	}
	/**
	 * 获取手机分辨率
	 * @return
	 ...
	 * @param input
	 */
	public void resetInput(String udid){
		DosCmd dc=new DosCmd();
		try {
			//			dc.execCmdConsole("adb shell \"ime set "+input+"\"");
			dc.execCmdConsole("adb -s "+ udid +" shell settings put secure default_input_method "+ inputs.get(udid));
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	 ...
	 * @param input
	 */
	public void switchInput(String udid,String input){
		DosCmd dc=new DosCmd();
		try {
